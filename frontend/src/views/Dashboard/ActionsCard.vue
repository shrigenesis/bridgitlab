...
const submitApplication = async () => {
  if (!formData.value.applicantName) {
    toast.error('Applicant name is missing');
    return;
  }
  ...
  if (!formData.value.applicantMobilePhoneNumber) {
    toast.error('Applicant Mobile number is missing');
    return;
  }
 
  if (!formData.value.applicantAddress) {
    toast.error('Applicant Address is missing');
    return;
  }
  if (!formData.value.incomingAddress) {
    toast.error('Incoming  Address is missing');
    return;
  }
  if (!formData.value.outgoingAddress) {
    toast.error('Outgoing  Address is missing');
    return;
  }
  ...
  if (response.success) toast.success('Application Saved Successfully.')
    ...
    toast.error(`loan's amount is above or below the average`)
...
<template>
  <div class="action-section">
    ...
    <BModal :visible="modal.isVisible.value" :confirm="modal.confirm">
      ...
      <template #footer>
        <BButton label="Submit" variant="primary" @click="submitApplication()"  ></BButton>
